...
from surveys.transform import group_average_by_question, question_score_range, wide_to_long
...
def test_wide_to_long_columns(tmp_path):
    assert _columns(wide_to_long(_csv(tmp_path))) == ["respondent_id", "question", "score"]
...
def test_group_average_columns(tmp_path):
    assert _columns(group_average_by_question(_csv(tmp_path))) == ["group", "question", "score"]


def test_group_average_values(tmp_path):
    result = {(r["group"], r["question"]): r["score"] for r in _records(group_average_by_question(_csv(tmp_path)))}
...
def test_question_score_range_columns(tmp_path):
    assert _columns(question_score_range(_csv(tmp_path))) == ["question", "min_score", "max_score"]
